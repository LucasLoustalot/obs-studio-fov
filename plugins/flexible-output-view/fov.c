...
static void frontend_event(enum obs_frontend_event event, void *data)
{
	...
	if (event == OBS_FRONTEND_EVENT_STUDIO_MODE_ENABLED) {
		...
		if (created != true) {
			...
			fov_app.fov_service =
				obs_service_create("rtmp_custom", "multitrack video service", service_data, NULL);
			...
			// Configuration du muxer, sortie vers un flux SRT
			...
			for (int i = 0; i < fov_app.nb_sources; i++) {
				...
				snprintf(encoder_name, sizeof(encoder_name), "FOV Track %d - %s", i,
					 obs_source_get_name(fov_app.source_refs[i]));
